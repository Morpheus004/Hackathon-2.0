...
function FileUpload({userInfo,all,pid}) {
    ...
    const fetchExistingFile = async () => {
      try {
        const response = await axios.get(
          `http://localhost:9000/file/check-file/${userInfo}`
        );
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:9000/file/download/${userInfo}`,
        {
          responseType: "blob",
        }
      );
